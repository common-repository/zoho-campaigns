...
var changevalue = function(selectedvalue) {
    ...
    function confirm_activate(val)
    {
        ...
        jQuery('#wc_disconnect_popup').slideUp();
    ...
    function mh_refresh_forms(){
        ...
        var data = {
            'action': 'zcwc_refresh_forms_list',
            'security': jQuery( '#mh-ajax-nonce' ).val(),
        };
        jQuery.post( ajaxurl, data, function(response) {
    ...
    function zcwc_setvalue(key,val)
    {
        jQuery("#zcwc_list").attr("value",key);
        jQuery("#zcwc_list").text(val);
